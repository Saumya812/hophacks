"""Add Pratyush as a reunited (found) demo case with the provided photo."""
...
SRC = Path(
    ...
    r"\c__Users_User_AppData_Roaming_Cursor_User_workspaceStorage_ecbe86187d29129beb28394ce5d11b10_images_image-b238445a-05df-48b8-9d07-2bbef24f4482.png"
)
...
def main() -> None:
    ...
    out = ROOT / "frontend" / "public" / "case-photos" / "pratyush.jpg"
    ...
    existing = next(
        (p for p in people if "pratyush" in (p.get("name") or "").lower()),
        None,
    )

    now = datetime.now(timezone.utc).isoformat()
    row = {
        "name": "Pratyush Paudel",
        "age": 19,
        "gender": "male",
        "last_seen_location": "Johns Hopkins University, Baltimore, MD",
        "last_seen_date": "2026-09-10",
        "description": (
            "Demo reunited case. Approx 5'10\", dark curly hair, glasses, last seen "
            "near Johns Hopkins Homewood campus wearing a dark blazer and light tie. "
        ...
        "found_at": now,
        "found_date": now,
        "found_message": "Found safe near Johns Hopkins. Reunited with family.",
        "found_notes": "Located near Johns Hopkins University campus; marked found for demo.",
        "verified_by": "law_enforcement",
        "external_case_number": "DEMO-FOUND-PRATYUSH",
