...
export default function Tip() {
  ...
  async function handleSubmit(e) {
    ...
    setError('')
    try {
      // datetime-local → ISO string for the API
    ...
    <div className="mx-auto max-w-xl space-y-6">
      <div>
        ...
        <p className="mt-2 text-navy/70">
          Share where and when you may have seen this person. Tips appear on the case map and timeline.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 border border-navy/10 bg-white/80 p-4 sm:p-6">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className="label-field" htmlFor="location_lat">Latitude *</label>
            <input
              id="location_lat"
              type="number"
              step="any"
          ...
          <div>
            <label className="label-field" htmlFor="location_lng">Longitude *</label>
            <input
              id="location_lng"
              type="number"
              step="any"
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {geoMsg && <span className="text-sm text-navy/60">{geoMsg}</span>}
        </div>

        <div>
          <label className="label-field" htmlFor="date_time">Date & time *</label>
          <input
            id="date_time"
            type="datetime-local"
            className="input-field"
        ...
        <div>
          <label className="label-field" htmlFor="description">What did you see? *</label>
          <textarea
            id="description"
